...
export function Header() {
    ...
    const { userAvatar, username } = useMetadata()
...

                {
                    user['session'] == null
                        ...
                        <div className="navegation flex gap-1 items-center relative">
                            <button style={{ backgroundImage: `${userAvatar ? `url(${userAvatar})` : null}` }} className={`avatar h-[40px] w-[40px] rounded-full ${userAvatar ? 'bg-cover bg-center bg-no-repeat' : 'bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500'}`} onClick={() => setActive(!active)}></button>
                            ...
                            <div className={`z-100 bg-white absolute h-[350px] w-[250px] top-12 right-0 shadow-xl rounded-lg flex flex-col py-8 ${active ? 'block' : 'hidden'}`}>
                                <p className="text-gray-600 border-b border-gray-300 text-center pb-2">{username}</p>
